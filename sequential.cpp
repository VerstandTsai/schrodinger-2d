    ...
    void step(double dt) {
        ...
        for (int i=1; i<_height-1; i++) {
            for (int j=1; j<_width-1; j++) {
                ...
                laplacian += _wave[k + _width];
                laplacian /= h2;
                _new_wave[k] = _wave[k] + -1i * laplacian * dt;
            }
        }
...
int main() {
    ...
    Simulation sim(sim_w, sim_h, 200, sim_w/4, sim_h/2, 0.01, 1, 0);
    while (!window.closed()) {
        sim.step(0.01);
        auto &wave = sim.wave();
